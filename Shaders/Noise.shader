...
uniform float vertical_factor = 3.0;
uniform float horizontal_factor = 4.0;
...
vec2 curve(vec2 uv, float time){
	
	uv = (uv - 0.5) * 2.0;
	
	uv.y *= 1.0 + pow(abs(uv.x) / sin(time)*0.5,2.0) * coeficient;
...
void fragment() {
    ...
    vec4 original_color = texture(SCREEN_TEXTURE, curve(SCREEN_UV,TIME));
    vec3 col = original_color.rgb;
    // If not greyscale
    vec3 hsb = rgb2hsb(col);
        // Shift the color by shift_amount, but rolling over the value goes over 1
    hsb.x = mod(hsb.x + abs(sin(TIME)) * coeficient, 1.0);
    col = hsb2rgb(hsb);
